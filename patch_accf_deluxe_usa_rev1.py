...
# ============================================================
#
# Offsets verified against rvfp-usa-rev1.xml and DOL analysis:
#
#   Hook RAM addr   : 0x8016BC90  file offset: 0x00166CB0
#   Hook instr      : 48344CE0 (b 0x804B0970)
#   Loader RAM addr : 0x804B0970  file offset: 0x004ACA70
#   wpadcl RAM addr : 0x80001800  (new DOL section)
#   CC patches      : from rvfp-usa-rev1.xml
#   Prc file        : E.bin (USA/NTSC)
#   Script folder   : Script/E/
# ============================================================
...
# RAM 0x8016BC90 (file 0x00166CB0) -> b 0x804B0970
# Original: 4BF493CD (bl 0x800B505C) - Nintendo dispatch code
# We replace it with our branch to the loader
dol[0x00166CB0:0x00166CB4] = bytes.fromhex("48344CE0")
print("  Hook written at 0x8016BC90 (file 0x00166CB0)")

# PATCH 2 - Inject loader.bin at 0x804B0970 (file 0x004ACA70)
# Zone of ~148KB of zeros, verified clean
...
# Source: rvfp-usa-rev1.xml
cc_patches = [
    (0x000F5644, "4BF071DC"),  # RAM 0x800FA624
    (0x000F56AC, "4BF0718C"),  # RAM 0x800FA68C
    (0x002C92A4, "48000010"),  # RAM 0x802CE284
    (0x0043EF6C, "4BBBD8E4"),  # RAM 0x80443F4C
    (0x0043EF88, "4BBBD8E0"),  # RAM 0x80443F68
    (0x003B99D8, "4BC42EA8"),  # RAM 0x803BE9B8
    (0x003BAA0C, "4BC41F44"),  # RAM 0x803BF9EC
]
...
# Script/E -> Script/ (USA region, {$__region} = E for NTSC-U)
...
print(f"  Hook at 0x8016BC90 (0x00166CB0): {'OK' if hook_ok else 'ERROR - ' + v[0x00166CB0:0x00166CB4].hex()}")
print(f"  Loader at 0x804B0970 (0x004ACA70): {'OK' if loader_ok else 'ERROR'}")
print(f"  module.kmdl at disc root: {'OK' if kmdl_ok else 'ERROR'}")
